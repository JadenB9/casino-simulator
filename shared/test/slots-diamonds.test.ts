...
describe('Diamond Line on the engine', () => {
  ...
  it('config: one seat, $1/$2/$5/$25/$100 coins or the high-limit $1,000, $5,000 and $10,000, 1-3 coins', () => {
    const cfg = engine.config('diamonds', 'solo');
    expect(cfg).toMatchObject({ game: 'slots', variant: 'diamonds', maxSeats: 1, limits: { default: { min: 100, max: 3_000_000, step: 100 } } });
    expect(cfg.options).toEqual({ machine: 'diamonds', denoms: [100, 200, 500, 2500, 10_000, 100_000, 500_000, 1_000_000], maxCoins: 3, lines: 1 });
  });
