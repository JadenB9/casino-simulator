...
describe('engine', () => {
  ...
  it('config: one seat, limits from the machine, unknown variants fall back to Classic Sevens', () => {
    // with the high-limit coins: up to $10,000 at Classic Sevens and 5x Wild ($30,000 a spin),
    // $500 at Neon Nights (five credits on twenty lines: $50,000); a hundred top bets to buy in
    expect(engine.config('sevens', 'solo')).toMatchObject({ game: 'slots', variant: 'sevens', maxSeats: 1, limits: { default: { min: 25, max: 3_000_000, step: 25 } } });
    expect(engine.config('neon', 'solo').limits.default).toEqual({ min: 100, max: 5_000_000, step: 100 });
    expect(engine.config('wild', 'solo').limits.default).toEqual({ min: 100, max: 3_000_000, step: 100 });
    expect(engine.config('sevens', 'solo').buyIn).toEqual({ min: 2_000, max: 300_000_000 });
    expect(engine.config('neon', 'solo').buyIn).toEqual({ min: 2_000, max: 500_000_000 });
  ...
  it('money adds up over thousands of spins on every machine, and nothing is ever left live', () => {
    for (const variant of ['sevens', 'neon', 'wild']) {
      ...
      const t = sim(variant, 10_000_000_000, seededRng(variant.length));
      ...
      expect(t.stack(0)).toBe(10_000_000_000 + net);
