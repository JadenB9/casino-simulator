...
export const paceBoost = { k: 1, /** v7.4: drunk, 0-1: the walk weaves */ stagger: 0, /** v7.4: crouched: a slow creep, no running */ crouch: false };
...
export class Player {
  ...
  update(dt: number): void {
    ...
    const boost = ride ? 1 : paceBoost.crouch ? 0.45 : paceBoost.k;
    const speed = len > 0 ? Math.min(MAX_PACE, (run && !(paceBoost.crouch && !ride) ? (ride?.run ?? RUN) : (ride?.walk ?? WALK) * pace) * boost) : 0;
    if (len > 0) {
      mx /= len;
      mz /= len;
    }
