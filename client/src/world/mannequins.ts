...
// faces and hands pale as a form's. Each is a character like any other (a draw call for the body,
// one or two for what it wears), hidden with the boutique when nobody can see it.
...
export class Mannequins {
  ...
  constructor(
    ...
    for (const f of plan.furniture) {
      ...
      const look: Look = {
        ...
        bottom: '#1c1c20',
        shoes: '#0c0c0e',
        ...(w.clothes ? { clothes: w.clothes } : {}),
        ...(w.chain ? { chain: w.chain } : {}),
        ...(w.grill ? { grill: w.grill } : {}),
        ...(w.watch ? { watch: w.watch } : {}),
        ...(w.hat ? { hat: w.hat } : {}),
        ...(w.shades ? { shades: w.shades } : {}),
      };
  ...
  async load(): Promise<void> {
    ...
    for (const m of this.people) {
      // settle into the idle pose, then stand still for good
      m.p.update(0.4);
