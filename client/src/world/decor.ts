...
import { BAR_TOP, CEILING, COFFEE_TABLE, COUCH, FLOOR_LAMP, LOUNGE_LAMP_X, PALM_PLANTER, PALM_WALK, PLANTER, PODIUM, STOOL, plantWalk, planterRadius, type FloorPlan } from './layout.ts';
...
export function buildDecor(plan: FloorPlan, stations: WorldStation[], b: Batch, m: Mats, col: Collider, glow: GlowMerge): Decor {
  ...
  for (const p of plan.palms) {
    planter(p.x, p.z, planterRadius('palm', p.size), PALM_PLANTER.h, PALM_WALK);
    out.props.push({ kind: 'palm', x: p.x, y: PALM_PLANTER.seat, z: p.z, ry: p.x, size: p.size });
  }
  for (const p of plan.plants) {
    planter(p.x, p.z, planterRadius(p.kind, p.size), PLANTER.h, plantWalk(p.kind, p.size));
