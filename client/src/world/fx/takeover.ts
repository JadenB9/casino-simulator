// Own the Night: the whole casino is the buyer's for a minute. Their name runs on the pit's LED
// sign (the Headline's program) and is thrown in gold light across the floor of every room from a
// gobo projector, the way a venue brands its floor for a night; shells burst under the ceiling
// wherever you are, gold, champagne and crimson stars falling slowly and crackling; and the gold of
// Golden Hour (the warm light, the shafts, the coins) fills every room under it all.
...
/** Stars in a shell, and shells a second on average. */
...
export function takeover(w: FxWorld, stock: Stock, ev: FxEvent, late: boolean, sign: () => Marquee | null): Effect {
  const q = w.quality();
  const parts = [headline(w, ev, late, sign), golden(w, stock, ev, late)];
  const stars = new Sparks(STARS[q] * 5, { hot: '#fffaf0', cool: '#ffffff', gain: q === 'high' ? 5 : 2.2, width: 0.03, len: 0.09, name: 'fx-fireworks' });
  const gobo = new THREE.Mesh(new THREE.PlaneGeometry(1, 1).rotateX(-Math.PI / 2), goboMaterial(ev.name));
  gobo.frustumCulled = false;
  w.root.add(stars.mesh, gobo);
  let goboRoom = '';
  let next = late ? 0.2 : 1.2;
  ...
  return {
    update(dt, t, left, view) {
      let alive = false;
      for (const p of parts) alive = p.update(dt, t, left, view) || alive;
      if (left > 2 && (next -= dt) <= 0) {
