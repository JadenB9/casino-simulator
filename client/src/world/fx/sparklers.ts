  ...
  return {
    update(dt, t, left) {
      ...
      units.forEach((u, i) => {
        ...
        const chase = t < 3 ? 1 : 0.55 + 0.45 * Math.max(0, Math.sin(t * 2.6 - (i / units.length) * Math.PI * 2));
        heat += chase;
        if (!burning) return;
      ...
      const k = burning ? (heat / Math.max(1, units.length)) * (0.85 + Math.random() * 0.3) : Math.max(0, left / 0.9) * 0.6;
