// Chips on the layout. Single player uses the casino's value chips; at a shared table every
// player gets roulette chips of their own colour (a real rule: no two players at a wheel may
// bet the same chips), stacked on top of each other on a shared spot. Materials are made once
// per denomination or colour and shared by every chip.
...
/** One colour per seat, picked to stay distinct from each other and from the red, black and green of the layout. */
...
/** Chips on this layout are drawn a little larger than life so they read against 12 cm boxes. */
...
/** A roulette ("wheel") chip: no value printed, just the player's colour with white inserts. */
...
export class LayoutChips {
  ...
  pile(seat: number, key: string): Pile | undefined {
  ...
  /** Height of everything stacked on a spot (for the marker and pills). */
