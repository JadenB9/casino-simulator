...
// pays is handed over too (v6 feats: shared/src/feats.ts), not won at play. v6 bank6: money in the
...
export function netStart(p: Profile | null, spent: Cents): NetStart {
  return { worth: p ? p.balance + p.inPlay + banked(p) : 0, loans: p?.loansTaken ?? 0, spent, feats: new Set((p?.feats ?? []).map((f) => f.feat)), bankGain: p?.bank?.gain ?? 0 };
}
...
export function sessionNet(p: Profile, start: NetStart, seat: { stack: Cents; escrow: Cents } | null, spent: Cents): Cents {
  ...
  return worth - start.worth - lent - rewards - bank + (spent - start.spent);
}
