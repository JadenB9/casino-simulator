...
await sleep(900);
const midA = await seen(b, a.id);
const midB = await seen(a, b.id);
check('B draws A walking', midA && midA.speed > 0.8, midA);
check('A draws B walking', midB && midB.speed > 0.8, midB);
...
if (await until(b, 'B hears A sat at dev-hc-1', (id) => window.presenceDev.remotes().find((r) => r.id === id)?.at?.station === 'dev-hc-1', a.id)) {
  await sleep(300);
  const sat = await seen(b, a.id);
  check('B draws A in the seat', sat.visible && Math.hypot(sat.x - -2.4, sat.z - 15.15) < 0.05, sat);
  await shot(b, 'presence-b-seated.png');
}
...
console.log(JSON.stringify({ ok: failed.length === 0 && errors.length === 0, checks, shots, errors: errors.slice(0, 10) }, null, 1));
